VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} Load_Sample_Annot 
   Caption         =   "Load_Sample_Annot"
   ClientHeight    =   7635
   ClientLeft      =   120
   ClientTop       =   465
   ClientWidth     =   12525
   OleObjectBlob   =   "Load_Sample_Annot.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "Load_Sample_Annot"
...
Private Sub Browse_Raw_Data_Click()
    ...
    If Raw_Data_File_Path.Text <> "" And Sample_Name_Text.Text = "" Then
        Load_Sample_Annot.Create_New_Sample_Annot_Button.Enabled = True
        Load_Sample_Annot.Merge_With_Sample_Annot_Button.Enabled = False
    End If
    
    If Raw_Data_File_Path.Text <> "" And Sample_Name_Text.Text <> "" Then
        Load_Sample_Annot.Create_New_Sample_Annot_Button.Enabled = False
        Load_Sample_Annot.Merge_With_Sample_Annot_Button.Enabled = True
    End If
...
Private Sub Browse_Sample_Annot_Click()
    ...
    If Load_Sample_Annot.Is_Column_Name_Present.Value = True Then
        ...
        Load_Sample_Annot.Column_Name_List.Clear
        For i = LBound(first_line) To UBound(first_line)
            Load_Sample_Annot.Column_Name_List.AddItem first_line(i)
        Next i
    Else
        ...
        For j = 1 To MaxColumn
            Load_Sample_Annot.Column_Name_List.AddItem "Column " & CInt(j)
        Next j
...
Private Sub Merge_With_Sample_Annot_Button_Click()
    ...
    Load_Sample_Annot.Hide
End Sub

Private Sub Create_New_Sample_Annot_Button_Click()
    whatsclicked = "Create_New_Sample_Annot_Button"
    Load_Sample_Annot.Hide
End Sub
